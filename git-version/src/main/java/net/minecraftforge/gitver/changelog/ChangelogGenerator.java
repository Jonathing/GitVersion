package net.minecraftforge.gitver.changelog;
...
public class ChangelogGenerator {
     ...
     *
     * @param git           The local repository from which to pull the git commit information.
     * @param repositoryUrl The github url of the repository.
     * @param justText      Indicates if plain text ({@code true}) should be used, or changelog should be used
     *                      ({@code false}).
     ...
     */
    public static String generateChangelogFromTo(final Git git, final String repositoryUrl, final boolean justText, final RevCommit start, final RevCommit end, final @Nullable String tagPrefix, final @Nullable String filter) throws GitAPIException, IOException {
        ...
        var changeLogName = git.getRepository().getFullBranch(); //Generate a changelog name from the current branch.
        ...
        changelog.append(justText ? "$changeLogName Changelog\n" : "### [$changeLogName Changelog]($repositoryUrl/compare/$startCommitHash...$endCommitHash)\n");
        ...
        for (final RevCommit commit : logList) {
            ...
            if (versionMap.containsKey(commitHash)) {
                ...
                commitHeader.append(tagMap.containsKey(commitHash) && !justText ? "[%s]%s".formatted(commitHeaderVersion, commitHeaderUrl) : commitHeaderVersion);
            }
